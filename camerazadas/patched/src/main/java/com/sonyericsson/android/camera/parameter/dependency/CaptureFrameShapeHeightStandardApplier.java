 ...
 */
public class CaptureFrameShapeHeightStandardApplier extends DependencyApplier {
    @Override
    public void apply(CapturingModeParams capturingModeParams) {
        if (capturingModeParams.mCapturingMode.get() == CapturingMode.SCENE_RECOGNITION || capturingModeParams.mCapturingMode.get() == CapturingMode.SUPERIOR_FRONT) {
            ParameterUtil.forceChange(capturingModeParams.mResolution, CaptureFrameShape.HEIGHT_STANDARD.getResolution(capturingModeParams.mCapturingMode.get().getCameraId()));
        }
    }
