...
public class NotificationManagerCompat {
    ...
    public static Set<String> getEnabledListenerPackages(Context object) {
        if (!((object = Settings.Secure.getString(object.getContentResolver(), "enabled_notification_listeners")) == null || object.equals(sEnabledNotificationListeners))) {
            Object object2 = object.split(":");
            HashSet<String> hashSet = new HashSet<String>(object2.length);
            int n = object2.length;
            for (int i = 0; i < n; ++i) {
                ComponentName componentName = ComponentName.unflattenFromString(object2[i]);
    ...
    private static boolean useSideChannelForNotification(Notification parcelable) {
        if ((parcelable = NotificationCompat.getExtras(parcelable)) != null && parcelable.getBoolean("android.support.useSideChannel")) {
            return true;
        }
        return false;
    }
    ...
    public void cancel(String string, int n) {
        IMPL.cancelNotification(this.mNotificationManager, string, n);
    ...
    public void notify(String string, int n, Notification notification) {
        ...
        IMPL.postNotification(this.mNotificationManager, string, n, notification);
    }
    ...
    implements Task {
        ...
        public CancelTask(String string) {
        ...
        public CancelTask(String string, int n, String string2) {
            this.packageName = string;
            this.id = n;
            this.tag = string2;
            this.all = false;
    ...
    implements Impl {
        ...
        @Override
        public void cancelNotification(NotificationManager notificationManager, String string, int n) {
        ...
        @Override
        public void postNotification(NotificationManager notificationManager, String string, int n, Notification notification) {
    ...
    extends ImplBase {
        ...
        @Override
        public void cancelNotification(NotificationManager notificationManager, String string, int n) {
            NotificationManagerCompatEclair.cancelNotification(notificationManager, string, n);
        }

        @Override
        public void postNotification(NotificationManager notificationManager, String string, int n, Notification notification) {
            NotificationManagerCompatEclair.postNotification(notificationManager, string, n, notification);
        }
    }
    ...
    implements Task {
        ...
        public NotifyTask(String string, int n, String string2, Notification notification) {
            this.packageName = string;
            this.id = n;
            this.tag = string2;
    ...
    ServiceConnection {
        ...
        private boolean ensureServiceBound(ListenerRecord listenerRecord) {
            ...
            listenerRecord.bound = this.mContext.bindService(intent, this, SIDE_CHANNEL_BIND_FLAGS);
            if (listenerRecord.bound) {
        ...
        private void processListenerQueue(ListenerRecord var1_1) {
            if (Log.isLoggable("NotifManCompat", 3)) {
                Log.d("NotifManCompat", "Processing component " + var1_1.componentName + ", " + var1_1.taskQueue.size() + " queued tasks");
            }
...
lbl-1000: // 1 sources:
            {
                if (Log.isLoggable("NotifManCompat", 3)) {
                    Log.d("NotifManCompat", "Sending task " + var2_2);
                }
...
lbl14: // 1 sources:
            ** GOTO lbl21
                ...
                if (Log.isLoggable("NotifManCompat", 3)) {
                    Log.d("NotifManCompat", "Remote service has died: " + var1_1.componentName);
                } else {
                    ** GOTO lbl21
                }
            }
            catch (RemoteException var2_4) {
                Log.w("NotifManCompat", "RemoteException communicating with " + var1_1.componentName, var2_4);
            }
lbl21: // 5 sources:
        ...
        private void scheduleListenerRetry(ListenerRecord object) {
            if (this.mHandler.hasMessages(3, object.componentName)) {
                return;
            }
            ++object.retryCount;
            if (object.retryCount > 6) {
                Log.w("NotifManCompat", "Giving up on delivering " + object.taskQueue.size() + " tasks to " + object.componentName + " after " + object.retryCount + " retries");
                object.taskQueue.clear();
                return;
            }
            int n = (1 << object.retryCount - 1) * 1000;
            if (Log.isLoggable("NotifManCompat", 3)) {
                Log.d("NotifManCompat", "Scheduling retry for " + n + " ms");
            }
            object = this.mHandler.obtainMessage(3, object.componentName);
            this.mHandler.sendMessageDelayed((Message)object, n);
        }
        ...
        private void updateListenerMap() {
            ...
            if (!object.equals(this.mCachedEnabledPackages)) {
                ...
                Map.Entry<ComponentName, ListenerRecord> entry2 = this.mContext.getPackageManager().queryIntentServices(new Intent().setAction("android.support.BIND_NOTIFICATION_SIDE_CHANNEL"), 4);
                HashSet<ComponentName> hashSet = new HashSet<ComponentName>();
                for (ResolveInfo resolveInfo : entry2) {
                ...
                for (Map.Entry<ComponentName, ListenerRecord> entry2 : hashSet) {
                    if (this.mRecordMap.containsKey(entry2)) continue;
                    if (Log.isLoggable("NotifManCompat", 3)) {
                        Log.d("NotifManCompat", "Adding listener record for " + entry2);
                    }
                    this.mRecordMap.put((ComponentName)entry2, new ListenerRecord((ComponentName)entry2));
                }
                object = this.mRecordMap.entrySet().iterator();
                while (object.hasNext()) {
                    entry2 = object.next();
                    if (hashSet.contains(entry2.getKey())) continue;
                    if (Log.isLoggable("NotifManCompat", 3)) {
                        Log.d("NotifManCompat", "Removing listener record for " + entry2.getKey());
                    }
                    this.ensureServiceUnbound(entry2.getValue());
                    object.remove();
                }
            }
        }
