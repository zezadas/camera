...
public class FitnessActivities {
     ...
     */
    public static int bp(String string) {
        int n = So.indexOfValue(string);
        if (n < 0 || (n = So.keyAt(n)) > 108) {
            return 4;
        }
        return n;
    }
    ...
    public static String getMimeType(String string) {
        return "vnd.google.fitness.activity/" + string;
    }

    public static String getName(int n) {
        String string = So.get(n);
        if (string == null) {
    ...
    public static void setValue(DataPoint dataPoint, String string) {
        dataPoint.getValue(Field.FIELD_ACTIVITY).setInt(FitnessActivities.bp(string));
    }
}
